...
marked.Renderer.prototype.link = function(href, title, text) {
  ...
  return this.origLink(href, title, text);
    ...
    marked.Lexer.prototype.token = function(src, top, bq) {
        ...
        this.rules.atStart = /^\@start(\w+)/;
        this.rules.atEnd = /\@end(\w+)/;
  ...
  while (src) {
    ...
    if (cap = this.rules.atStart.exec(src)) {
      src = src.substring(cap[0].length);
      let token = {
          type: 'start',
          lang: cap[1]
...
MdRender.prototype.parseAndRender = function(src, relPath) {
    ...
    extTokens = tokens.map((t) => {
        if (t.type === 'start') {
            id = md5(t.text);
            if (!this.extensionData[id]) {
                this.extensionData[id] = { source: t.text };
            }
            return { 
                type: 'paragraph',
